...
{
    if (badgeString == nil) {
        NSAssert(badgeString==nil, @"badgeString is nil");
    }

    CGSize retValue;
    CGFloat rectWidth, rectHeight;
    CGSize stringSize = [badgeString sizeWithFont:[UIFont boldSystemFontOfSize:14]];
    CGFloat flexSpace;
    ...
    //	self.frame = CGRectMake(self.frame.origin.x-2, self.frame.origin.y-2, retValue.width-15, retValue.height);//原来的
    self.frame = CGRectMake(36,-2, retValue.width-15, retValue.height);//写死的
    if (self.frame.size.width < 18)
    {
        self.frame = CGRectMake(40,0,18,18);
    }
    self.badgeText = badgeString;
    [self setNeedsDisplay];
}
